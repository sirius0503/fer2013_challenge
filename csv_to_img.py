...
def main():
    if len(sys.argv) < 2:
        print('Usage: python3 csv_to_img.py [output_path]')
        return -1
    ...
    os.system('mkdir {}'.format(output_path)) # os.makedirs os.makedirs(output_path)
    
    label_names = ['Angry', 'Disgust', 'Fear', 'Happy', 'Sad', 'Surprise', 'Neutral']
    #data = np.genfromtxt('fer2013.csv', delimiter=',', dtype=None)
    data = pd.read_csv('fer2013.csv', header=None)
    labels = data.iloc[1:, 0].astype(np.int32)
    image_buffer = data.iloc[1:,1]
